...
### 동작
1. **신호 탐지** — 전일대비 수익률·갭·거래량을 과거 60일 대비 z-score화하여 **z>2.5** 초과분을 score 합산 + 52주 고·저가 시 +0.5. score >= 1.0이면 이벤트, 같은 섹터 30% 동반 시 섹터 이벤트로 판단한다.
2. **원인 추적** — 탐지 종목의 최근 뉴스·최신 공시 자동 수집 및 분석한다.
