...
- 신호별 향후 수익률이 시장(baseline) 대비 유의한지 부트스트랩으로 검정하였다.  
    ...
    KR - 유의미한 평균 차이 없음.  
    US - 이벤트 발생 이후 60일 기준으로 모든 지표에서 시장 대비 평균 초과수익률이 긍정적 신호에서는 양의 방향, 부정적 신호에서는 음의 방향을 나타내었다. 통계적 유의성을 테스트하기 위한 부트스트랩 검정 결과 60일 기준으로 8개의 지표 중 주가 상승, 갭 상승, 거래량 동반 하락 세 지표는 95% 신뢰구간이 0을 포함하였고 나머지 지표는 뚜렷한 초과수익률의 방향을 나타내었다. 특히 52주 고·저가 지표가 가장 큰 방향성을 보였다.

    <img src="results/img/exc_by_signal_US.png" width="800">

    <img src="results/img/bootstrap_ci_60d_US.png" width="800">
...
- **결론**  
  ...
  과반의 설정지표가 시장 대비 통계적으로 유의한 초과/미달 수익을 보여 추세 전환 포착 신호로서의 유효성을 확인했다. 일부 신호는 유의성까지 도달하지는 못했으나 긍정 신호는 양(+), 부정 신호는 음(-)의 평균 초과수익이라는 일관된 방향성을 보였다. 
  -> 지표 설계의 방향성은 타당했다고 판단했다. 

  다만 생존편향 혹은 표본의 불충분성과 같은 한계가 존재하므로 설정된 지표가 무조건적인 초과수익률을 보장할 수 있다는 결론은 내릴 수 없다.
